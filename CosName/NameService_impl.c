...
#ifdef LWIP
#include "lwip/sys.h"
static sys_mutex_t GLOBAL_SOCK_MUTEX = 0;
#define SOCKET_LOCK() do{ if(GLOBAL_SOCK_MUTEX == 0){sys_mutex_new(&GLOBAL_SOCK_MUTEX);} sys_mutex_lock(&GLOBAL_SOCK_MUTEX);} while(0)
#define SOCKET_UNLOCK() sys_mutex_unlock(&GLOBAL_SOCK_MUTEX)
